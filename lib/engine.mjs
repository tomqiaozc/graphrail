...
export function synthesizeGate(sessionDir, state = readState(sessionDir)) {
  const entries = [];
  ...
  for (const entry of entries) {
    ...
    for (const artifact of handshake.artifacts || []) {
      ...
      if (artifact.type === "evaluation") {
        ...
        try { evaluationVerdict = JSON.parse(text).verdict; } catch {
          evaluationVerdict = text.match(/\bVERDICT\b[^A-Z\n]{0,24}(PASS|ITERATE|FAIL|BLOCKED)\b/i)?.[1];
        }
...
export function transition(sessionDir, verdict, { manual = false } = {}) {
  return withLock(resolve(sessionDir, ".lock"), () => {
    ...
    if (node.kind === "gate") {
      const synthesis = synthesizeGate(sessionDir, state);
...
export function advance(sessionDir) {
  const state = readState(sessionDir);
  if (state.template.nodes[state.currentNode].kind === "gate") {
    const synthesis = synthesizeGate(sessionDir, state);
    if (synthesis.reasons.length) throw new Error(`gate rejected: ${synthesis.reasons.join("; ")}`);
    return transition(sessionDir, synthesis.verdict);
  }
  const handshake = validateCurrentEvidence(sessionDir, state);
...
export function finalize(sessionDir) {
  return withLock(resolve(sessionDir, ".lock"), () => {
    ...
    const provenance = new Ledger(sessionDir, resolve(sessionDir, "provenance.json"));
    const chainCheck = provenance.verifyChain();
    if (!chainCheck.valid) throw new Error(`cannot finalize with invalid provenance chain: ${chainCheck.errors[0]}`);
    const finalizeEntry = provenance.append({ kind: "finalize", flow: state.flowId, steps: state.totalSteps, finalizedAt: new Date().toISOString() });
    const tail = provenance.tail();
    // Write a signed chain head so a later verifier can check the chain was
    // closed at the exact head, and resume can only append after that head.
    atomicJson(resolve(sessionDir, "chain-head.json"), {
      schemaVersion: 2,
      eventHash: tail.eventHash,
      signature: finalizeEntry.signature,
