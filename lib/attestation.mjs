...
export function recordClaudeEvent(sessionDir, input) {
  ...
  if (ledger.schemaVersion !== 1 || !Array.isArray(ledger.events)) throw new Error("unsupported adapter event ledger");
  for (const event of ledger.events) {
    if (!validSignature(sessionDir, event)) throw new Error("adapter event signature mismatch");
  }
  if (ledger.events.some((event) => event.eventId === input.eventId)) return { recorded: false, duplicate: true };
  const event = signed(sessionDir, {
    schemaVersion: 1,
...
export function validateReviewAttestation(sessionDir, nodeId, runId, artifact) {
  const errors = [];
  let binding;
  try { binding = readClaudeAdapter(sessionDir); } catch (error) { return [error.message]; }
  const ledgerPath = resolve(sessionDir, EVENTS_PATH);
  if (!existsSync(ledgerPath)) return ["review evidence has no trusted Claude lifecycle ledger"];
  const ledger = readJson(ledgerPath);
  if (ledger.schemaVersion !== 1 || !Array.isArray(ledger.events)) return ["unsupported adapter event ledger"];
  const events = ledger.events;
  if (events.some((event) => !validSignature(sessionDir, event))) return ["adapter event signature mismatch"];
  ...
  if (start === -1) errors.push(`reviewer ${artifact.agentRunId} has no trusted start event`);
  if (start !== -1 && relevant[start].agentType !== "graphrail-reviewer") errors.push(`agent ${artifact.agentRunId} is not a trusted graphrail-reviewer`);
  if (write === -1) errors.push(`evaluation ${artifact.path} is not bound to a reviewer write event`);
  if (stop === -1) errors.push(`reviewer ${artifact.agentRunId} has no trusted completion event`);
  if (start !== -1 && write !== -1 && write < start) errors.push(`reviewer ${artifact.agentRunId} wrote evidence before start`);
  if (write !== -1 && stop !== -1 && stop < write) errors.push(`reviewer ${artifact.agentRunId} completed before writing evidence`);
  if (failure !== -1 && (stop === -1 || failure < stop)) errors.push(`reviewer ${artifact.agentRunId} failed before trusted completion`);
  return errors;
}
