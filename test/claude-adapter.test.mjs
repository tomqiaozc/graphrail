...
`);
  ...
  assert.equal(readJson(resolve(dir, "adapter-events.json")).entries.length, 1);
  ...
  assert.equal(readJson(resolve(dir, "adapter-events.json")).entries.length, 2);
  ...
  assert.equal(agents["graphrail-test-designer"].model, "sonnet");
